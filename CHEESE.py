...
st.title("🧀 CHEESE — CO₂ Handling & Electrolyzer Engineering Scale-up Evaluator")
...
# -------------------- Interface and sidebar controls --------------------
...
GAS_FLOW_UNIT = st.sidebar.selectbox(
    ...
    help="All gas inputs and outputs use this display unit. Calculations remain on a standard dry-flow basis.",
)
...
if IS_Simple:
    n_units_global = st.sidebar.number_input(
        ...
        step=1,
        key="gs_units",
        help="Use 1 for a single cell or enter the number of identical cells in the stack.",
    )
    use_stack_global = n_units_global > 1
    LIQUID_FLOW_UNIT = "mg/h"

    with st.sidebar.expander("Calculation assumptions", expanded=False):
    ...
    gas_pressure_drop_bar = 0.0
    liquid_pressure_drop_bar = 0.0
else:
    ...
    use_stack_global = st.sidebar.checkbox(
        "Use a stack (multiple identical units)?",
        value=True,
        key="gs_stack",
    )
    n_units_global = st.sidebar.number_input(
...
# -------------------- Materials cost per test --------------------
def cost_per_area_to_per_cm2(cost_value: float, cost_unit: str) -> float:
    """Convert a user-entered area-normalized material cost to $/cm²."""
...
def make_cell_stack_figure(n_cells: int, active_area_cm2: float) -> go.Figure:
    ...
    fig.add_annotation(
        ...
        text=f"{subtitle}<br>Active area per cell: {active_area_cm2:,.1f} cm²",
...
def render_cost_per_test() -> None:
    ...
    st.subheader("Materials Cost per Electrolyzer Test")
    st.caption(
        "Estimate the anode, cathode, and membrane cost allocated to one test. "
        "The calculation is area-based and supports either a single cell or a repeated stack."
    )

    n_cells = int(n_units_global if use_stack_global else 1)

    st.markdown("### 1. Define the cell or stack")
    c1, c2, c3 = st.columns(3)
    with c1:
        tea_area_value = st.number_input(
    ...
    with c2:
        material_area_factor = st.number_input(
            "Purchased-area factor",
            min_value=1.0,
            value=1.0,
            step=0.05,
            key="tea_material_area_factor",
            help=(
                "Purchased or cut material area divided by active area. "
                "Use 1.00 when the priced area equals the active area; use a larger value for borders, seals, or cutting waste."
            ),
        )
    with c3:
    ...
    with anode_col:
        ...
        anode_cost_unit = st.selectbox(
            "Anode cost basis",
            options=["$/cm²", "$/m²"],
            index=0,
            key="tea_anode_cost_unit",
        )
        anode_reuse = st.number_input(
            "Tests per anode",
            min_value=1,
            value=1,
            step=1,
            key="tea_anode_reuse",
            help="Use 1 for a single-use anode. Larger values prorate the purchase cost across repeated tests.",
        )

    with membrane_col:
        st.markdown("#### Membrane")
        membrane_cost = st.number_input(
            "Membrane cost",
            min_value=0.0,
            value=0.0,
            step=0.01,
            key="tea_membrane_cost",
        )
        membrane_cost_unit = st.selectbox(
            "Membrane cost basis",
            options=["$/cm²", "$/m²"],
            index=1,
            key="tea_membrane_cost_unit",
        )
        membrane_reuse = st.number_input(
            "Tests per membrane",
            min_value=1,
            value=1,
            step=1,
            key="tea_membrane_reuse",
            help="Use 1 when a fresh membrane is installed for every test.",
        )

    with cathode_col:
        st.markdown("#### Cathode")
        cathode_cost = st.number_input(
            "Cathode cost",
            min_value=0.0,
            value=0.0,
            step=0.01,
            key="tea_cathode_cost",
        )
        cathode_cost_unit = st.selectbox(
            "Cathode cost basis",
            options=["$/cm²", "$/m²"],
            index=0,
            key="tea_cathode_cost_unit",
        )
        cathode_reuse = st.number_input(
            "Tests per cathode",
            min_value=1,
            value=1,
            step=1,
            key="tea_cathode_reuse",
    ...
    with st.expander("Other per-test material costs", expanded=False):
        other_consumables = st.number_input(
            ...
            value=0.0,
            step=1.0,
            key="tea_other_consumables",
        )
        st.caption(
            "Enter only costs consumed by one test. Durable pumps, flow controllers, potentiostats, and cell hardware are not included in this simple model."
        )
    ...
    rows.append({
        "Component": "Other consumables",
        "Entered cost": other_consumables,
        "Entered basis": "$/test",
        "Cost ($/cm²)": np.nan,
        "Purchased area per item (cm²)": np.nan,
        "Items in test": 1,
        "Full purchase cost ($)": other_consumables,
        "Tests per item": 1,
        "Allocated cost per test ($)": other_consumables,
    })

    cost_df = pd.DataFrame(rows)
    total_cost_per_test = float(cost_df["Allocated cost per test ($)"].sum())
    direct_stack_purchase = float(cost_df.loc[cost_df["Component"] != "Other consumables", "Full purchase cost ($)"].sum())
    cost_per_cell_test = total_cost_per_test / max(n_cells, 1)
    cost_per_active_cm2 = total_cost_per_test / max(total_active_area_cm2, EPS)
    cost_df["Share of test cost (%)"] = (
        100.0 * cost_df["Allocated cost per test ($)"] / total_cost_per_test
        if total_cost_per_test > EPS
        else 0.0
    )

    st.markdown("### 3. Materials-cost result")
    m1, m2, m3, m4 = st.columns(4)
    with m1:
        st.metric("Materials cost per test", f"${total_cost_per_test:,.2f}")
    with m2:
        st.metric("Cost per cell per test", f"${cost_per_cell_test:,.2f}")
    with m3:
        st.metric("Full new layer set", f"${direct_stack_purchase:,.2f}")
    with m4:
        st.metric("Cost / total active area", f"${cost_per_active_cm2:,.4f}/cm²")
    ...
    if total_cost_per_test > EPS:
        chart_df = cost_df.loc[cost_df["Allocated cost per test ($)"] > EPS].copy()
        cost_bar = alt.Chart(chart_df).mark_bar().encode(
            x=alt.X("Allocated cost per test ($):Q", title="Allocated cost per test ($)"),
            y=alt.Y("Component:N", sort="-x", title=None),
            tooltip=[
                "Component:N",
                alt.Tooltip("Allocated cost per test ($):Q", format="$,.2f"),
                alt.Tooltip("Share of test cost (%):Q", format=".1f"),
            ],
        ).properties(height=260, title="Per-test material cost breakdown")
        cost_text = alt.Chart(chart_df).mark_text(align="left", baseline="middle", dx=4).encode(
            x="Allocated cost per test ($):Q",
            y=alt.Y("Component:N", sort="-x"),
            text=alt.Text("Allocated cost per test ($):Q", format="$,.2f"),
        )
        st.altair_chart(cost_bar + cost_text, use_container_width=True)

        top_row = chart_df.loc[chart_df["Allocated cost per test ($)"].idxmax()]
        st.info(
            f"Largest current cost driver: **{top_row['Component']}** at "
            f"${top_row['Allocated cost per test ($)']:,.2f}/test "
            f"({top_row['Share of test cost (%)']:.1f}% of the modeled total)."
        )
    else:
        st.info("Enter at least one material or consumable cost to generate the cost breakdown.")
    ...
    with st.expander("View calculation table and equations", expanded=False):
        ...
        st.dataframe(
            ...
            use_container_width=True,
            column_config={
                "Entered cost": st.column_config.NumberColumn(format="$%.4f"),
                "Cost ($/cm²)": st.column_config.NumberColumn(format="$%.6f"),
                "Purchased area per item (cm²)": st.column_config.NumberColumn(format="%.2f"),
                "Full purchase cost ($)": st.column_config.NumberColumn(format="$%.2f"),
                "Allocated cost per test ($)": st.column_config.NumberColumn(format="$%.2f"),
                "Share of test cost (%)": st.column_config.NumberColumn(format="%.1f%%"),
            },
        )
        st.download_button(
            "Download materials cost breakdown (CSV)",
            data=cost_df.to_csv(index=False).encode("utf-8"),
            file_name="cheese_materials_cost_per_test.csv",
            mime="text/csv",
            use_container_width=True,
            key="tea_download_cost_csv",
        )

    st.warning(
        "Scope: this is a screening-level materials cost, not a full techno-economic analysis. "
        "It excludes labor, electricity, gas supply, analytical charges, depreciation, maintenance, and durable test-stand hardware unless those items are manually included under other consumables."
    )
...
def auto_format_liquid_mass_rate(kg_h: float) -> Tuple[str, float, str]:
    ...
    if kg_h >= 1.0:
        ...
        value, unit = kg_h * 1e3, "g/h"
    else:
        value, unit = kg_h * 1e6, "mg/h"
...
def Simple_product_rate(core: Dict[str, float], product: str) -> Tuple[str, str]:
    ...
    n_product = core.get(f"{product}_mol_s", 0.0)
    kg_h, _ = mflow_to_mass_and_vol(
        n_product,
        props["MW (g/mol)"],
        props["ρ_liq (kg/L)"],
    )
    formatted, _, unit = auto_format_liquid_mass_rate(kg_h)
...
def Simple_fe_status(primary_fe_pct: float, h2_fe_pct: float) -> bool:
    ...
    st.progress(int(round(min(max(assigned_fe, 0.0), 100.0))))

    if assigned_fe > 100.0 + 1e-9:
        st.error(
            f"Assigned FE is {assigned_fe:.1f}%. The total cannot exceed 100%. "
            "Reduce the primary-product FE or H₂ FE."
        )
        return False

    unassigned_fe = max(0.0, 100.0 - assigned_fe)
    if unassigned_fe <= 1.0:
        st.success(f"Assigned FE: {assigned_fe:.1f}%. The current balance is essentially closed.")
    elif unassigned_fe <= 10.0:
        st.info(
            f"Assigned FE: {assigned_fe:.1f}%. Unassigned current: {unassigned_fe:.1f}%. "
            "CHEESE does not assign this remainder to another product."
        )
    else:
        st.warning(
            f"Assigned FE: {assigned_fe:.1f}%. Unassigned current: {unassigned_fe:.1f}%. "
            "Check whether another product should be included in Advanced Mode."
        )
    return True
...
def render_Simple_mode() -> None:
    ...
    st.info(
        "Start by choosing the question you want CHEESE to answer. "
        "Only the essential assumptions are shown; specialist corrections and sensitivity tools remain available in Advanced Mode."
    )
    ...
    carbon_products = [
        ...
        if PRODUCT_MAP[product]["co2_per_mol"] > 0
    ]
    ...
    if Simple_task == "production":
        ...
        carbon_per_amp_factor = sum(
            ...
            * PRODUCT_MAP[product]["co2_per_mol"]
            / max(PRODUCT_MAP[product]["nₑ⁻ to product"], EPS)
            for product in PRODUCT_LIST
        )

        if carbon_per_amp_factor <= EPS:
            st.error("The selected FE assumptions do not produce a carbon-containing product.")
            return
        ...
        r1, r2, r3 = st.columns(3)
        with r1:
            st.metric("Required area per cell", f"{area_per_cell_cm2:,.1f} cm²")
        with r2:
    ...
    with st.expander("How was this calculated?", expanded=False):
        st.markdown(
            "CHEESE first converts current into product molar flow using Faraday's law. "
            "It then applies the selected product stoichiometry and CO₂ utilization."
        )
        st.latex(r"\dot{n}_i=\frac{I\,FE_i}{n_{e,i}F}")
    ...
    with st.expander("Terms used in Simple Mode", expanded=False):
        st.markdown(
            ...
            "- **CO₂ utilization:** fraction of inlet CO₂ incorporated into carbon products in this balance.\n"
            "- **Stoichiometric ratio, S:** inlet CO₂ divided by the theoretical product-forming CO₂ requirement.\n"
            "- **Standard gas flow:** dry flow reported at the selected STP or SATP molar-volume basis."
        )
